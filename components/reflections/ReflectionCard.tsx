...
export function ReflectionCard({ reflection }: ReflectionCardProps) {
  ...
  return (
    <Link href={`/reflections/${reflection.id}`}>
      <div className="group relative cursor-pointer overflow-hidden rounded-xl border border-purple-500/20 bg-gradient-to-br from-slate-900/90 to-slate-800/90 p-6 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:border-purple-500/50 hover:bg-white/10 hover:shadow-lg hover:shadow-purple-500/20">
        {/* Premium indicator */}
        ...
        {reflection.title && (
          <div className="mb-3">
            <span className="inline-block rounded-full border border-purple-500/40 bg-purple-500/20 px-3 py-1 text-xs font-medium text-purple-300">
              {reflection.title}
            </span>
          </div>
        )}
        ...
        <div className="mb-4 flex items-center gap-3 text-xs text-white/40">
          ...
          <span>•</span>
          <span
            className={`inline-flex items-center rounded-full border px-2.5 py-0.5 font-medium capitalize ${getToneBadge(reflection.tone)}`}
          >
            {reflection.tone}
          </span>
        </div>
        ...
        <p className="mb-4 line-clamp-3 text-sm leading-relaxed text-white/80">{snippet}</p>
        ...
        <div className="mt-4 flex items-center gap-2 text-sm text-purple-400 opacity-0 transition-opacity group-hover:opacity-100">
