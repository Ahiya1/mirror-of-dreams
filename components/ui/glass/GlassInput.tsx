    ...
    <div className="space-y-2">
      {label && (
        <label htmlFor={id} className="block text-sm font-medium text-white/70">
          {label}
          {required && <span className="ml-1 text-mirror-error">*</span>}
        </label>
      )}
      <div className="relative">
        ...
        {showCounter && maxLength && actualType === 'textarea' && (
          <motion.div
            className="pointer-events-none absolute bottom-3 right-3 text-xs font-medium"
            variants={
      ...
      {/* Error Message */}
      {error && (
        <p className="flex items-center gap-1 text-sm text-mirror-error">
