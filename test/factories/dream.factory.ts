// test/factories/dream.factory.ts - Dream test data factory
// Provides reusable dream factories for testing Dreams router
 ...
 * @example
 * ```typescript
 * const dream = createDreamForUser('user-123', { title: 'My Goal' });
 * ```
 */
 ...
 * @example
 * ```typescript
 * const dreams = createFreeTierDreams('free-user-001');
 * ```
 */
 ...
 * @example
 * ```typescript
 * const dreams = createProTierDreams('pro-user-001');
 * ```
 */
 ...
 * @example
 * ```typescript
 * const achievedDream = createDreamWithStatus('achieved');
 * ```
 */
...
/**
 * Dream with stats interface (for list operations)
 */
export interface DreamWithStats extends DreamRow {
 ...
 *
 * @param overrides - Partial dream data to merge including stats
 * @returns Dream with stats fields
 *
 * @example
 * ```typescript
 * const dreamWithStats = createMockDreamWithStats({ reflectionCount: 5 });
 * ```
 */
...
/**
 * Dream limits per subscription tier
 */
