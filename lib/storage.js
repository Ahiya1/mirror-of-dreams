...
function calculateStats(receipts, gifts = []) {
  ...
  const todayReceipts = receipts.filter((r) => new Date(r.timestamp).toDateString() === today);

  const todayGifts = gifts.filter((g) => new Date(g.createdAt).toDateString() === today);
...
async function loadReceipts() {
  try {
    ...
    console.log(`📋 Loaded ${receipts ? receipts.length : 0} receipts from Redis`);
    return receipts || [];
  } catch (error) {
...
async function getReceiptsByEmail(email) {
  const receipts = await loadReceipts();
  return receipts.filter((r) => r.customerEmail.toLowerCase() === email.toLowerCase());
}
